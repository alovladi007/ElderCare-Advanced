...
const MonitoringLoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:24613/api/auth/login', formData);
  ...
  const handleQuickLogin = async (email, password) => {
    ...
    try {
      const response = await axios.post('http://localhost:24613/api/auth/login', { email, password });
        ...
        >
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 inline-block">
            ...
            <p className="text-blue-200 text-sm mt-2">
              Backend should be running on http://localhost:24613
            </p>
          </div>
        </motion.div>
      </div>
